 ...
 * handshake with auth.nishchinto.com.bd (the centralized source of truth).
 */
export function SSOHandshakeIframe() {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (typeof window !== "undefined") {
         ...
         if (event.data?.type === "SSO_TOKEN_SYNC") {
           ...
           Cookies.set("nishchinto_jwt", event.data.token, { expires: 1, domain: window.location.hostname });
  ...
  return (
    <iframe
      src={process.env.NEXT_PUBLIC_AUTH_URL || "https://auth.nishchinto.com.bd/sso-hub"}
