...
export function BlockRenderer({ theme, shopInfo }: BlockRendererProps) {
    ...
    const showcaseVariant = theme?.active_components?.product_showcase || "grid";
    ...
    return (
        <>
            ...
            <main className="flex-1">
                ...
                <ProductShowcaseBlock variant={showcaseVariant} shopSlug={shopInfo.subdomain} />
            </main>
...
function HeaderBlock({ variant, shopName }: { variant: string, shopName: string }) {
    if (variant === "minimal") {
        return (
    ...
    if (variant === "center_logo") {
        return (
            <header className="border-b h-20 flex flex-col items-center justify-center">
                <div className="text-2xl font-bold tracking-widest">{shopName}</div>
...
function HeroBlock({ variant, shopName }: { variant: string, shopName: string }) {
        ...
        return (
            <section className="h-[60vh] bg-slate-900 text-white flex flex-col items-center justify-center text-center p-8 bg-[url('/placeholder-hero.jpg')] bg-cover bg-center">
                <div className="bg-black/50 p-8 rounded-xl backdrop-blur-sm">
                    ...
                    <p className="text-lg opacity-80 max-w-xl">Welcome to our official store. Discover the best products handpicked for you.</p>
                </div>
            </section>
        );
    }
    if (variant === "minimal_typography") {
        return (
            <section className="py-32 px-8 max-w-4xl mx-auto text-center">
                <h1 className="text-6xl font-light tracking-tighter mb-6">{shopName}.</h1>
                <p className="text-xl text-muted-foreground leading-relaxed">Curated essentials for your everyday life. We focus on quality and timeless design.</p>
            </section>
        );
    }
    // Split Media
    return (
        <section className="grid grid-cols-1 md:grid-cols-2 min-h-[500px]">
            <div className="flex flex-col justify-center p-12 lg:p-24 bg-muted/30">
                <h1 className="text-4xl md:text-5xl font-bold mb-6 tracking-tight">Welcome to {shopName}</h1>
                <p className="text-lg text-muted-foreground mb-8">Discover our latest collection. High quality products tailored for your needs.</p>
                <div>
                    <button className="bg-primary text-primary-foreground px-8 py-3 rounded-md font-medium">Shop Now</button>
                </div>
            </div>
            <div className="bg-muted flex items-center justify-center overflow-hidden">
                <div className="text-muted-foreground/50 font-mono">Hero Image Placeholder</div>
            </div>
        </section>
    );
}

function ProductShowcaseBlock({ variant, shopSlug }: { variant: string, shopSlug: string }) {
    ...
    if (variant === "carousel") {
        return (
            <section className="py-16 overflow-hidden pl-8">
                <h2 className="text-2xl font-bold mb-8 px-4">Featured Products</h2>
                <div className="flex gap-6 overflow-x-auto pb-8 snap-x">
                    {placeholders.slice(0, 5).map(i => (
                        <div key={i} className="min-w-[300px] snap-start group cursor-pointer">
                            ...
                            <div className="text-muted-foreground text-sm">৳ 1,200</div>
                        </div>
                    ))}
                </div>
            </section>
    ...
    return (
        <section className="py-16 px-8 max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-8">
                ...
                <a href="#" className="text-sm font-medium hover:underline">View All &rarr;</a>
            </div>
                ...
                {placeholders.map(i => (
                    <div key={i} className="group cursor-pointer">
                        ...
                        <div className="flex justify-between items-start">
                            <div>
                                <h3 className="font-medium">Premium Item {i}</h3>
...
function FooterBlock({ variant, shopName }: { variant: string, shopName: string }) {
    if (variant === "expanded") {
        return (
                ...
                <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
                    <div className="md:col-span-1">
                        ...
                        <p className="text-sm opacity-80 leading-relaxed mb-6">
                            High quality products delivered right to your door. Secure payments and fast shipping.
                        </p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            <input type="email" placeholder="Email" className="bg-slate-800 px-3 py-2 rounded text-sm w-full outline-none focus:ring-1 ring-primary" />
                            <button className="bg-primary text-primary-foreground px-4 py-2 rounded text-sm font-medium">Join</button>
                        </div>
                    </div>
                </div>
    ...
    return (
        <footer className="border-t py-12 px-8 text-center text-muted-foreground">
            ...
            <p className="text-xs">&copy; {new Date().getFullYear()} {shopName}. Powered by Nishchinto.</p>
        </footer>
    );
}
